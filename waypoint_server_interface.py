...
tour, avg_pts, connections = container_extraction.connect_faces(
    cpc.raster_paths)
final_connections = container_extraction.make_full_path(
    tour, avg_pts, cpc.raster_paths, connections, cpc.container_surface_corners)
...
class WaypointServerInterface:
    ...
    def register_drone_with_server(self, drone_name, drone_type, drone_topics):
        ...
        """
        register_service = roslibpy.Service(self.client,
        ...
        save_topics_service = roslibpy.Service(
            self.client, 'isaacs_server/save_drone_topics',
    ...
    def send_waypoints(self, drone_id, waypoints):
        ...
        """
        service = roslibpy.Service(self.client, 'isaacs_server/upload_mission',
                                   'isaacs_server/upload_mission')
        satellite_points = []
        for point in waypoints:
            satellite_point = {}
            satellite_point['latitude'], satellite_point[
                'longitude'], satellite_point['altitude'] = point
            satellite_points.append(satellite_point)

        request = roslibpy.ServiceRequest({
            'id': drone_id,
            'waypoints': satellite_points
        })
...
if __name__ == "__main__":
    ...
    topics_published = [{'name': 'test_name', 'type': 'std_msgs/String'}]
    waypoints = [(1, 0, 0), (1, 2, 3), (4, 5, 6)]

    wsi = WaypointServerInterface()
    drone_id = wi.register_drone_with_server(drone_name, drone_type,
                                             topics_published)
